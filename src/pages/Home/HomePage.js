...
import "./Estilo_Inicio_Baixo_Navbar.css";
import "./Estilo_Navbar.css";

import Imagem_Home from "../../img/Imagem_Home.png";

import Abertura_Inicial from "../../components/Ferramentas/Abertura_Inicial";

export default function HomePage() {
  const Navegar = useNavigate();

  const [Aberto_Primeria_Vez, setAberto_Primeria_Vez] = useState();

  useEffect(() => {
    if (Cookies.get("Site_Ja_Aberto_Pela_Primeira_Vez")) {
      setAberto_Primeria_Vez(false);

      setTimeout(() => {
        document.querySelectorAll(".Geral").forEach((item) => {
          item.style.opacity = 1;
        });
      }, 100);
    } else {
      setAberto_Primeria_Vez(true);
      setTimeout(() => {
        document.querySelectorAll(".Geral").forEach((item) => {
          item.style.opacity = 1;
        });
      ...
      setTimeout(() => {
        Cookies.set("Site_Ja_Aberto_Pela_Primeira_Vez", "false", { path: "/" });
      }, 1000);
    }
  }, []);
  ...
  return (
    <div className="Corpo_Site">
      {(Aberto_Primeria_Vez == true) | (Aberto_Primeria_Vez == false) ? (
        <>
          {Aberto_Primeria_Vez && (
            <Abertura_Inicial
              Elemento_Que_Vai_Renderizar={
                <svg
                  version="1.0"
          ...
          <div class="Geral geral_style" style={{ opacity: 0 }}>
            <nav>
              <div class="logo">
                <svg
                  version="1.0"
